...
public class StudentMapper {
    ...
    public List<Student>selectByPage(int begin, int pageSize) throws SQLException, NoSuchFieldException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        String sql = "select * from student limit ?,?";
        List<Student> list = curd.Query(Student.class, sql, begin, pageSize);
        if (list.isEmpty() || list == null) {
            return null;
        }
        return list;
    ...
    public List<Student>selectByPageAndCondition(int begin, int pageSize, String studentNumber, String studentName, String gender) throws SQLException, NoSuchFieldException, NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException {
        StringBuilder condition = new StringBuilder("select * from student where 1=1");
        List<Object> params = new ArrayList<>();

        if (studentNumber != null && !studentNumber.equals("")) {
            condition.append(" and student_number like ? ");
        ...
        if (gender != null && !gender.equals("")) {
            ...
            condition.append(" and gender = '" + sex+"'");
        }
        condition.append("  limit ?,? ");
        params.add(begin);
        params.add(pageSize);

        List<Student> list = curd.Query(Student.class, condition.toString(), params.toArray());

        return list;
    }

    public int selectCountByPageAndCondition(String studentNumber, String studentName, String gender) throws SQLException, NoSuchFieldException, NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException {
        StringBuilder condition = new StringBuilder("select * from student where 1=1");
        List<Object> params = new ArrayList<>();
